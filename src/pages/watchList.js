    ...
    return(
            ...
            watchLater && watchLater.length>0? (
                <div className="flex sm:justify-center sm:w-3/4 sm:mt-32 mt-20 w-full sm:p-0 p-2">
                    <div className="flex sm:justify-center justify-between w-full">
                        {
                            watchLater.map((movie,index)=>(
                                <Poster movie={movie} key={index} classContent={"sm:w-40 sm:h-60 h-42 w-32 shrink-0 sm:mr-3"} source={`https://image.tmdb.org/t/p/w500${movie.poster_path}`}></Poster>
                            ))
                        }
                    </div>
                </div>
